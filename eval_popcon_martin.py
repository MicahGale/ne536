...
def calc_martin_scaling(params):
    P_LH=0.0488*params['ne']**0.72*params['B_tor']**0.8*params['S']**0.94
    return(P_LH)
...
def main(params=None):
    ...
    params['S'] =  4.*np.pi**2 * params['R'] * params['a_eff']
    ...
    nne=10
    ne_arr=np.linspace(0.2,1.2,num=nne,endpoint=True)
    nT=11
    ...
    P_LH_arr=np.zeros((nne,nT))
    P_abs_arr=np.zeros((nne,nT))
    ...
    plt.figure()
    levels=10**np.linspace(0,np.log10(np.max(P_abs_arr)),num=5)
    #levels=[86]
    cs2=plt.contour(T_target_arr,ne_arr,P_abs_arr,colors='k',levels=levels)
    ...
    #levels=10**np.linspace(0,np.log10(np.max(P_heat_arr)),num=5)
    levels=[33.]
    cs3=plt.contour(T_target_arr,ne_arr,P_heat_arr,colors='r',levels=levels)
    plt.clabel(cs3,inline=True,inline_spacing=5, fmt="%.0f", fontsize=9)

    cs3=plt.contour(T_target_arr,ne_arr,P_LH_arr,colors='green',levels=5)
    plt.clabel(cs3,inline=True,inline_spacing=5, fmt="%.0f", fontsize=9)


    handles = [plt.Line2D([0], [0], color="k", lw=1, label=r'$P_{HCD}+P_\alpha$ [MW]'),
        plt.Line2D([0], [0], color="blue", lw=1, label="Q"),
        plt.Line2D([0], [0], color="r", lw=1, label=r'$P_{HCD}$ [MW]'),
        plt.Line2D([0], [0], color="green", lw=1, label=r'$P_{LH}$ [MW]'),]
